...
export class PlaidService {
  ...
  public async fetchLatestBalance(userId: string) {
    const plaidTokens: PlaidToken[] =
      await this.prismaService.plaidToken.findMany({
        where: {
          userId: userId
        }
      });
  ...
  public async onPlaidSuccess(
    data: Prisma.AccountCreateManyInput[],
    plaidToken: any
  ) {
    try {
      ...
      this.dataGatheringService.handleUpdateHoldingsInvestment(
        plaidToken['accessToken']
      );
      console.log('Importing Holdings Done.....');

      console.log('handleUpdateTranscationInvestment ......');
      this.dataGatheringService.handleUpdateTranscationInvestment(
        plaidToken['accessToken']
      );
      ...
      if (
        data[0] &&
        data[0].verification_status === 'pending_automatic_verification'
      ) {
        return {
      ...
      if (
        data[0] &&
        data[0].verification_status === 'pending_manual_verification'
      ) {
        return {
      ...
      case 'ERROR':
        {
          ...
          const isPlaidMessageExist =
            await this.prismaService.plaidMessages.findFirst({
              where: {
                itemId: bodyData.item_id
              }
            });
      ...
      case 'AUTOMATICALLY_VERIFIED':
        {
          ...
          const isAccount_idExist = await this.prismaService.account.findFirst({
            where: {
              account_id: account_id
            }
          });
  ...
  public async getPlaidMessages(userId: string) {
    try {
      ...
      const sanatizedData = [];
      plaidTokens.map(
        ({ PlaidMessages, institutionUniqueId, accessToken, itemId }) => {
          if (PlaidMessages.length !== 0) {
            PlaidMessages.map(({ item_Login_Required_Status }) => {
              sanatizedData.push({
                item_Login_Required_Status,
                institutionUniqueId,
                accessToken,
                itemId
              });
  ...
  public async updateInvestmentsTransactions(bodyData) {
    ...
    if (plaidToken && plaidToken['accessToken']) {
      ...
      await this.dataGatheringService.handleUpdateHoldingsInvestment(
        plaidToken['accessToken']
      );
